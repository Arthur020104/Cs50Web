...
def newproduct(request):
    ...
    if request.method == "POST":
        if not request.POST["productname"] and request.POST["productprice"] and request.POST["productabout"] and request.POST["img"]:
            return render(request, "auctions/NewProduct.html", {
                "message":"All fields must be filled"
            })
        else:
            p = Product(userwining = request.user,bids_count = 0, current_bid = float(request.POST["productprice"]), creator = request.user, img = request.POST["img"], name = request.POST["productname"], about = request.POST["productabout"], time_created = datetime.date.today(), status = "Active")
            p.save()
            return HttpResponseRedirect(reverse("index"))
